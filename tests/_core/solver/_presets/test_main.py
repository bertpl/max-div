...
# Each preset (by resolved alias, so DEFAULT follows SMART) yields this init for an unconstrained problem.
_EXPECTED_INIT_UNCONSTRAINED: dict[SolverPreset, type[InitializationStrategy]] = {
    SolverPreset.RANDOM: InitRandomSelection,
    SolverPreset.GUIDED: InitRandomSelection,
...
@pytest.mark.parametrize("preset", list(SolverPreset))
def test_get_preset_strategies_init_on_a_constrained_problem(preset: SolverPreset):
    """SMART/THOROUGH start from most_feasible() when the problem has constraints."""
