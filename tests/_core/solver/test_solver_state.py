...
def test_solver_state_end_to_end(new_solver_state):
    ...
    assert np.array_equal(state.global_signal_array, state._signal_tracker.global_signal)
    ...
    assert np.allclose(state.selected_signal_array, [2, 2, 3])
    ...
    assert np.allclose(state.selected_signal_array, [2, 2, 2])
    assert np.allclose(state.not_selected_signal_array, [1, 1, 1])
    assert state.score.size == 1.0  # correct number of vectors selected
    assert state.score.constraints == 1.0  # all constraints satisfied
...
def test_solver_state_snapshot(new_solver_state):
    ...
    orig_separation_array = state.selected_signal_array.copy()
    ...
    assert np.allclose(state.selected_signal_array, orig_separation_array)
...
def test_solver_state_consistency_stress_test(new_solver_state, seed: int):
    ...
    assert np.array_equal(state.global_signal_array, state_ref.global_signal_array)
    assert np.array_equal(state.not_selected_signal_array, state_ref.not_selected_signal_array)
    assert np.array_equal(state.selected_signal_array, state_ref.selected_signal_array)
...
def _assert_state_matches_fresh_rebuild(state: SolverState) -> None:
    ...
    assert np.array_equal(state._signal_tracker._sep_selected, fresh._signal_tracker._sep_selected)
