...
| distance metrics · single-dimension distance | <span class="mark mark-partial">◐</span> | [^highs-2] |
| distance metrics · caller-supplied distances | <span class="mark mark-full">✔</span> | [^highs-3] |
| diversity objectives · maximize the minimum separation | <span class="mark mark-partial">◐</span> | [^highs-4] |
| diversity objectives · maximize the mean nearest-neighbor separation | <span class="mark mark-partial">◐</span> | [^highs-5] |
| diversity objectives · maximize the geometric-mean nearest-neighbor separation | <span class="mark mark-partial">◐</span> | [^highs-5] |
| diversity objectives · maximize the mean pairwise distance | <span class="mark mark-partial">◐</span> | [^highs-4] |
...
| solver scaling · largest n closing 90% of the quality gap | n = 200 | |
...
[^highs-2]: Reachable by handing the tool the one coordinate as one-dimensional input: in one dimension every Minkowski distance is the absolute difference of that coordinate.
[^highs-3]: Distances are coefficients you supply, so an unusual metric costs no more than a common one.
[^highs-4]: Reachable, but the objective must be linearized by hand — for max-min, a threshold variable bounded below every selected pair via big-M constraints.
[^highs-5]: Reachable only through an assignment MILP that pairs each selected item with its nearest selected neighbor; the formulation is considerably larger than the max-min one and is what drives the practical size limit down.
