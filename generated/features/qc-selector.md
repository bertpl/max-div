...
| distance metrics · single-dimension distance | <span class="mark mark-partial">◐</span> | [^qc-selector-5] |
| distance metrics · caller-supplied distances | <span class="mark mark-full">✔</span> | [^qc-selector-6] |
| diversity objectives · maximize the minimum separation | <span class="mark mark-full">✔</span> |  |
| diversity objectives · maximize the mean nearest-neighbor separation | <span class="mark mark-none">—</span> | [^qc-selector-7] |
| diversity objectives · maximize the geometric-mean nearest-neighbor separation | <span class="mark mark-none">—</span> | [^qc-selector-7] |
...
| constraints beyond k · per-group counts over disjoint groups | <span class="mark mark-full">✔</span> | [^qc-selector-8] |
...
| time budget · budget expressed as an iteration count | <span class="mark mark-partial">◐</span> | [^qc-selector-9] |
| time budget · budget expressed as wall-clock time | <span class="mark mark-none">—</span> |  |
| time budget · the answer improves when given more budget | <span class="mark mark-none">—</span> | [^qc-selector-10] |
| multi-worker · several workers search one problem separately and the best result wins | <span class="mark mark-none">—</span> | [^qc-selector-11] |
...
[^qc-selector-5]: Reachable by handing the tool the one coordinate as one-dimensional input: in one dimension every Minkowski distance is the absolute difference of that coordinate.
[^qc-selector-6]: A precomputed distance matrix is accepted directly, so any metric you can compute is usable.
[^qc-selector-7]: Its diversity measures are computed over the whole selection rather than over nearest-neighbor pairs, so the nearest-neighbor family is absent.
[^qc-selector-8]: Label-stratified selection: given class labels, it picks proportionally across them. That covers disjoint groups with proportional targets, but not arbitrary minimum and maximum counts, and not groups an item can belong to more than once.
[^qc-selector-9]: Several of its methods take a parameter that controls how much work they do — a sphere-exclusion radius, an OptiSim subsample size — but these change the character of the search rather than lengthening it. Raising one does not mean a better answer.
[^qc-selector-10]: A single construction pass, so there is no budget to spend: the answer is whatever one greedy sweep produces, and waiting longer does not change it.
[^qc-selector-11]: MaxMin and MaxSum selection is sequential; OptiSim and DISE parallelize only their KDTree neighbor queries, not the search over selections.
