"""The upper-logarithmic axis transform, for values that approach a limit from below."""
...
class UpperLogTransform(AxisTransform):
    ...
    A score that saturates spends most of a solve within a sliver below its final value, and a linear
    axis draws that sliver as a flat line. This transform stretches it: the axis coordinate is
    ...
    def ticks_and_labels(self, n_max: int = 20, should_add_missing_ticks: bool = False) -> tuple[list[float], list[str]]:
        ...
        repeat. The number of ticks is searched upward from `n_max` until rounding leaves as close to
        ...
        for n_max_internal in range(n_max, 10 * n_max):
            ...
            if len(candidate_ticks) == n_max:
                ...
                break
            elif len(candidate_ticks) <= n_max:
                ticks, labels = candidate_ticks, candidate_labels
    ...
    def from_values(cls, x: ArrayLike, n_candidates: int = 100) -> UpperLogTransform:
        ...
        [0, 1] wins. Constant data gets a transform with `x_ref` one above the value and the value itself
        drawn at 0.5.
        ...
        if x_min == x_max:
            # c1 < 0 keeps the axis increasing in x, as every fitted transform is
        ...
        for x_ref in x_ref_candidates:
            ...
            x_axis = c0 + c1 * np.log(x_ref - x_sorted)
            # uniform data has its q-quantile at q; the sum of squared deviations measures the distance
    ...
    def _rounded_ticks_and_labels(self, n_ticks: int) -> tuple[list[float], list[str]]:
        ...
        # --- round each position to the precision of the room around it ---
        labels: list[str] = []
        for x, x_axis in zip(positions, positions_axis, strict=True):
            room = float(self.from_axis(x_axis + 0.5 * step_axis)) - float(self.from_axis(x_axis - 0.5 * step_axis))
            n_digits = max(1, -int(math.floor(math.log10(room)))) - 1
