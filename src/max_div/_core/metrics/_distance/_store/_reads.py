...
The single owner of layout knowledge: which half of a full matrix to touch, and when a distance is
computed rather than looked up.  Everything else treats a store as opaque.
"""
...
# is named after.  A reader handed another layout reads a zero-length array, which is fatal rather
# than wrong.  Settled once per tracker, by looking the reader up by store kind.
...
def get_distance(store: DistanceStore, i: np.int32, j: np.int32) -> np.float32:
    ...
    For reads of a single pair.  Loops reading many pairs use the layout-specific readers above,
    ...
    order (the shape all tracker kernels follow).  The full-matrix backend then reads contiguous
