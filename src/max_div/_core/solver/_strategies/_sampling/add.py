...
def select_items_to_add(
    ...
    include_within_group_signal: bool | np.bool_ = True,
    ...
    :param include_within_group_signal: (bool) flag that influences how sampling probabilities are built.
                                            True: start from signal wrt already selected items + within-group signal
                                            False: start from signal wrt already selected items only
    ...
    if state.n_selected == 0:
        # the only option is to look at the global signal (wrt all other items), as we don't have a selection yet
        # this branch is only taken in the first iteration of initialization strategies
        p = state.global_signal_array[candidates]  # signal of candidates wrt all other items
    else:
        # standard path
        p = state.full_signal_array[candidates]  # new array; signal of candidates wrt selected items
        if include_within_group_signal:
            p += state.global_signal_array[candidates]  # add signal of candidates wrt all other items

    exponential_selectivity(
        p_in=p,
        p_out=p,  # in-place
        modifier=np.float32(selectivity_modifier),
        reverse=False,  # for adding, we want to have vectors with high diversity signal have higher probability
    )
