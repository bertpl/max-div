...
class InitFixedSelection(InitializationStrategy):
    ...
    The indices are checked in 2 stages: the constructor checks the indices on their own, and
    `check_fits_problem_size` checks them against the problem's n and k.
    """
    ...
    def _as_checked_indices(indices: ArrayLike) -> NDArray[np.int64]:
        ...
        values, counts = np.unique(index_array, return_counts=True)
        if np.any(counts > 1):
