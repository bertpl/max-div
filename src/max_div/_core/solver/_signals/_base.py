...
# =================================================================================================
#  DiversitySignalTracker
# =================================================================================================
class DiversitySignalTracker(ABC):
    """Tracks each point's per-point diversity signal wrt an incrementally changing selection.

    The *diversity signal* of a point is a value monotone in its marginal contribution to the
    diversity objective: for a selected point, how much it contributes to the current selection's
    diversity; for a non-selected point, how much diversity it would add if selected.  Higher is
    always more diverse.  Each concrete tracker defines the signal of one diversity-metric family
    and owns the arrays + kernel calls that maintain it incrementally.
    ...
    # -------------------------------------------------------------------------
    @abstractmethod
    def full_signal(self, selected: NDArray[np.bool], n_selected: np.int32) -> NDArray[np.floating]:
        """Return per-point signal of all n points wrt the current selection.
    ...
    @property
    @abstractmethod
    def global_signal(self) -> NDArray[np.floating]:
        """Return static per-point signal of each point wrt the whole dataset (selection-independent).
    ...
    def add(self, index: np.int32) -> None:
        """Update signals after adding point `index` to the selection."""
        raise NotImplementedError

    @abstractmethod
    def remove(self, index: np.int32, new_selection: NDArray[np.int32]) -> None:
        """Update signals after removing point `index` from the selection.
        ...
        :param new_selection: (int32 ndarray) indices selected *after* the removal — needed by
                              trackers whose signal requires rescanning the remaining selection.
        """
        raise NotImplementedError

    def add_many(self, indices: NDArray[np.int32]) -> None:
        """Update signals after adding all points in `indices` to the selection."""
        for index in indices:
            self.add(index)

    def remove_many(self, indices: NDArray[np.int32], new_selection: NDArray[np.int32]) -> None:
        """Update signals after removing all points in `indices` from the selection.
    ...
    def set_snapshot(self) -> None:
        """Internally save the current signal state, overwriting any previous snapshot."""
        raise NotImplementedError

    @abstractmethod
    def restore_snapshot(self) -> None:
        """Restore the signal state saved by the last `set_snapshot` call and invalidate it."""
        raise NotImplementedError
