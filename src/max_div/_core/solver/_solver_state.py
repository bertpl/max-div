...
class SolverState:
    ...
    def __init__(
        ...
        con_membership: dict[np.int32, NDArray[np.int32]] | None,
    ) -> None:
        ...
        Args:
            ...
            con_membership: (dict[np.int32, NDArray[np.int32]] | None) mapping from index to its constraints
                (as generated by _build_con_membership); None when the problem has no constraints, so an
    ...
    def new(
        ...
        con_membership = _build_con_membership(n_np, constraints) if constraints else None
...
def _build_con_membership(
    ...
    """Build a mapping from each of the n item indices to the list of constraints it belongs to."""
    # map index -> list
    con_membership_lst: dict[np.int32, list[np.int32]] = {i: [] for i in np.arange(n, dtype=np.int32)}
