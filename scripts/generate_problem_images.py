...
def _titled_figure(title: str, subtitle: str) -> tuple[plt.Figure, plt.Axes]:
    ...
    fig.text(0.06, 0.955, title, fontsize=14, fontweight="bold", ha="left")
    fig.text(0.06, 0.92, subtitle, fontsize=11, ha="left")
    ax.set_xlabel("x1")
    ax.set_ylabel("x2")
...
def _crosshair_gridlines(ax: plt.Axes) -> None:
    ...
    for value in (0.0, 1.0):
        ax.axhline(value, color=LIGHT_GRAY, linestyle="--", linewidth=0.7, alpha=0.7, zorder=0)
        ax.axvline(value, color=LIGHT_GRAY, linestyle="--", linewidth=0.7, alpha=0.7, zorder=0)
...
def _draw_boundary(ax: plt.Axes, name: str) -> None:
    ...
    if name in ("U1", "U2"):
        ax.plot([0, 1, 1, 0, 0], [0, 0, 1, 1, 0], color=LIGHT_GRAY, linewidth=1.0, label="Population boundary")
    if name == "U1":
        theta = np.linspace(0.0, 2.0 * np.pi, 200)
        for radius in (0.72, 0.82):
            ax.plot(0.5 + radius * np.cos(theta), 0.5 + radius * np.sin(theta), color=LIGHT_GRAY, linewidth=1.0)
    if name == "U3":
        theta = np.linspace(0.0, 2.0 * np.pi, 200)
        ax.plot(2.0 * np.cos(theta), 2.0 * np.sin(theta), color=LIGHT_GRAY, linewidth=1.0, label=r"2$\sigma$ contour")
    if name == "U4":
        ...
        ax.plot(center[0] + r * np.cos(arc), center[1] + r * np.sin(arc), color=LIGHT_GRAY, linewidth=1.0)
...
def _draw_band_constraints(ax: plt.Axes, name: str, n: int) -> None:
    ...
    if name in ("C1", "C2"):
        for i in range(m + 1):
            ax.axvline(
                i / m, color=BLUE, linestyle="-.", linewidth=1.0, label="Constraint group boundary" if i == 0 else None
            )
        ...
        for i in range(m):
            ...
            ax.text(x_mid, y_text * 0.86, label, color=BLUE, fontsize=9, ha="center", va="bottom")
    if name in ("C3", "C4"):
        # per-dimension sign constraints split each axis at 0
        ax.axvline(0.0, color=BLUE, linestyle="-.", linewidth=1.0, label="Constraint boundary")
        ax.axhline(0.0, color=BLUE, linestyle="-.", linewidth=1.0)
    if name == "C4":
        # the additional per-dimension band constraint covers [-1, +1]
        for value in (-1.0, 1.0):
            ax.axvline(value, color=BLUE, linestyle=":", linewidth=0.8)
...
def render_geometry(name: str, with_solution: bool) -> None:
    ...
    ax.scatter(problem.vectors[:, 0], problem.vectors[:, 1], s=19, color=GRAY, label="Population", zorder=2)
    ...
    ax.legend(loc=_LEGEND_LOC[name], framealpha=0.9)
