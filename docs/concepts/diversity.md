...
| `along_axis(axis)` | $$d = \lvert x_{\text{axis}} - y_{\text{axis}} \rvert$$ | The distance along one coordinate axis, every other coordinate ignored. A selection kept apart under it is spread along that single coordinate; it is also the per-coordinate building block behind the geometric-mean distance. |
