...
```
Vectors  ──>  Pairwise Distances  ──>  Diversity Signals  ──>  Diversity Score
...
2. A **diversity signal** is computed for each selected vector -- a per-vector quantity where
   higher means "contributes more diversity". Which signal depends on the diversity metric's
   family:
...
3. The **diversity score** aggregates the `k` signal values into a single scalar using the
...
- **`MEAN_PAIRWISE_DISTANCE`** is the objective to pick when you want classical max-sum
  ...
  solver's swap heuristics were originally designed and tuned around separation signals; they
  are correct for this metric (its per-vector signal is the vector's exact marginal
