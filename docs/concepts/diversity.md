...
| `along_axis(axis)` | $$d = \lvert x_{\text{axis}} - y_{\text{axis}} \rvert$$ | The distance along one coordinate axis, every other coordinate ignored. A selection kept apart under it is spread along that single coordinate. The geometric-mean distance is the product of this distance over every axis, to the power $1/d$. |
