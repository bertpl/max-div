...
Each worker's `WorkerSummary` carries its derived seed next to the configuration it ran, and when it
started on the shared time axis (`t_start_offset_sec`, zero for the earliest worker). For an
