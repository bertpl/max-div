...
**time budget** (`seconds()`, `minutes()`, `hours()`), a **budget for the whole solve**
(`total_seconds()`, `total_minutes()`, `total_hours()`), or an **iteration count**
(`iterations()`).
...
# Or the same two minutes for the whole solve
solver = (
    MaxDivSolverBuilder(problem)
