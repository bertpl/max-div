...
| `HARMONIC_MEAN_SEPARATION` | Harmonic mean of all separations | When a close pair should weigh more than under the geomean, without the tied scores of the minimum |
...
By default the budget covers the **optimization only** — computing the distances and initializing come on top. `with_end_to_end_budget()` changes that:

- the same time budget covers the **whole solve**, and the optimization gets whatever time remains after setup — the store build and initialization have no budgets of their own (they always run to completion), so the optimization step is the only place remaining time can be applied;
