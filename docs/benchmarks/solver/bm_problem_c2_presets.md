...
These results compare the built-in solver presets on problem C2, run at n=1000 — the size at which it selects k=100 items, so every problem's page compares presets at the same selection size. Each preset is run for a ladder of increasing time budgets, with a fresh random seed at every budget point, so the spread across seeds is visible alongside the time trend.
...
![Problem C2 - Preset results](./images/preset_results_C2_1000.webp)

### B. Tables

--8<-- "docs/benchmarks/solver/results/preset_result_quantiles_C2_1000.md"
