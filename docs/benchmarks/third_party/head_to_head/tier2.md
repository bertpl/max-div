...
- **Objective**: minimum separation under the `L2` distance, scored identically for every tool by `max-div`'s own metric code. Tools that optimize a different objective — `apricot-select` (facility location), `kmedoids` (representativeness), `DPPy` (a determinantal sample), the max-sum picker of `qc-selector` — enter as different-objective references, not as dispersion competitors.
- **Entrants**: every non-exact registry tool, at the sizes its scaling time limit covers; one run per seed where the tool is seeded. A tool's time includes any conversion it needs. Exact solvers are the [exact-solver tier](tier1.md)'s.
- **max-div**: `DEFAULT` preset, two budget series per size — one worker from 1 ms to 60 s (15 points) and 12 workers with the default dynamic grouping from 1 s to 60 s (6 points) — one independent solve per budget and seed, one solve at a time, timed end to end around the call. Charts plot *measured* wall-clock, never the nominal budget.
...
- **n = 10,000**: `max-div`'s first budgets sit 0.3 % below the picker line, the optimization overtakes at 0.5 s, and 60 s end 8 % (one worker) and 9 % (12 workers) above it. The picker itself takes 2 ms (`fpsample[KDLine]`) to 4 s (`RDKit`) for its one answer.
