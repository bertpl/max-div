...
- a positive $\alpha$ moves the largest item further out, growing the largest separation by a factor $10^{\alpha}$;
- a negative $\alpha$ moves the smallest item toward its neighbor, shrinking the smallest separation by the same factor.
...
- **min separation** and **harmonic-mean separation** follow the smallest separation and do not react when the largest one grows tenfold: the harmonic mean is the number of separations divided by the sum of their reciprocals, and the smallest separations dominate that sum;
