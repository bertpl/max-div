...
from .helpers import ProblemDimensions, benchmark_init_strategy, get_problem_dimensions


# =================================================================================================
#  Main function
# =================================================================================================
def estimate_init_time_model(
    name: str,
    init_strat_factory: Callable[[], InitializationStrategy],
) -> InitializationTimeModel:
    # --- init ----------------------------------
    problem_dims: list[ProblemDimensions] = get_problem_dimensions()

    # --- benchmark -----------------------------
    benchmark_data = {
        dim: benchmark_init_strategy(
            problem=dim.build_problem(),
            init_strat_factory=init_strat_factory,
        )
        for dim in tqdm(problem_dims, desc=f"Benchmarking init strategy '{name}'")
    }
...
def _x_to_time_model(x: np.ndarray) -> InitializationTimeModel:
    return InitializationTimeModel(
        t0=float(10 ** x[0]),
        t0_k=float(10 ** x[1]),
        t_n=float(10 ** x[2]),
        t_k=float(10 ** x[3]),
        t_m=float(10 ** x[4]),
        t_n_con_indices=float(10 ** x[5]),
    )
