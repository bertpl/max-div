...
### Added
- `with_initial_selection(...)` on the solver builders starts a solve, or every worker of a parallel solve, from a given selection of k items (a hot start); `InitializationStrategy.fixed_selection(...)`, passed as `WorkerConfig(init_strategy=...)`, gives a worker its own starting selection
