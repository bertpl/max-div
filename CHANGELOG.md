...
- Constrained sampling is roughly 9× faster per swap-candidate call and 90× per initialization-sized call on heavily overlapping constraint structures — constraint-satisfaction scores are now maintained incrementally instead of recomputed from scratch per draw; selections are unchanged
