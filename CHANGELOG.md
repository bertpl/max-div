...
### Changed
- Random one-shot initialization updates its distance bookkeeping in one batched pass — an order of magnitude faster at large n, with a `parallel` option for another large factor when the solve runs single-process
