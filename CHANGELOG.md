...
### Changed
- Distance-store construction is multi-threaded, roughly 7× faster (depending on CPU architecture) at large problem sizes; results are unchanged (parallel builds are bit-identical to the single-threaded ones), and `MAXDIV_PARALLEL_BUILD=0` disables it
