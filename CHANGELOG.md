...
### Fixed
- A solve's checkpoint times and `duration` now include the solver's own overhead between steps, so a parallel solve no longer places one worker's events out of order relative to another's
...
### Added
- `with_initial_selection(...)` on the solver builders starts a solve, or every worker of a parallel solve, from a given selection of k items (a hot start); `InitializationStrategy.given_selection(...)`, passed as `WorkerConfig(init_strategy=...)`, gives a worker its own starting selection
...
### Changed
- The preset benchmark series (`max-div benchmark solver presets`) tops out at 900 s instead of 1800 s
