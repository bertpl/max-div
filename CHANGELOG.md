...
- `InitializationStrategy.farthest_point` now also does what `farthest_point_batched` did: when every term of the diversity objective is a separation-family metric over one distance, it draws many items per pass over the dataset (its new `batch_size` parameter caps the candidates per pass; `batch_size=None` draws 1 item per pass), and its `top_k` defaults to 8, where it was 1
