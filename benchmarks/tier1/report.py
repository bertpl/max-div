...
# Budget points quoted in the gap table (seconds; must be actual values of the budget series).
...
def maxdiv_gap_pct(records: list[RunRecord], metric_name: str, budget_sec: float, optimum: float) -> float:
    """Mean gap (%) of max-div to a proven optimum at one budget point (positive = below optimum)."""
...
def build_scaling_table(rows: list[dict]) -> str:
    ...
    for n in ns:
        ...
        for backend in backends:
            ...
            if row is None:
                cells.append("—")  # budget series stopped earlier for this backend
            elif row["proven"]:
