"""Time-ceiling stage: every tool ascends the size grid at the reference budget.
...
from benchmarks.ceilings.configs import TOOLS, Mode, seeds_for
from benchmarks.ceilings.grid import REFERENCE_BUDGET_SEC, operational_bound, size_grid
from benchmarks.ceilings.records import append_ceiling_record, load_ceiling_records
...
def run_time_stage(tools: list[str], data_path: Path = DATA_PATH) -> dict[str, int | None]:
    """Run the stage for the given tools and return each one's time ceiling.
    ...
    """
        ...
        (r.tool, r.n, r.seed): r
        for r in (load_ceiling_records(data_path) if data_path.exists() else [])
        if r.mode == Mode.FASTEST_VALID.value
    }
    ceilings: dict[str, int | None] = {}
    for tool in tools:
        ceilings[tool] = _ascend(tool, done, data_path)
        print(f"{tool}: time ceiling {ceilings[tool]}")
    return ceilings
    ...
    for n in size_grid(operational_bound()):
        ...
        for seed in seeds_for(tool):
            ...
            if record is None:
                ...
                append_ceiling_record(record, data_path)
