...
def test_best_competitor_ignores_the_random_baseline():
    # --- arrange -----------------------------------------
    records = [
    ...
    # --- act / assert ------------------------------------
    assert report.best_competitor(records) == ("RDKit[MaxMinPicker]", pytest.approx(1.2), pytest.approx(0.5))
...
def test_main_emits_chart_per_size_with_tables(tmp_path: Path):
    ...
    save_records([_record("fpsample[FPS]", "single-shot", 1.0)], data_dir / report.COMPETITOR_FILE)
    save_records(
    ...
    assert "| fpsample[FPS] |" in (docs_dir / "results" / "tier2_competitors.md").read_text()
