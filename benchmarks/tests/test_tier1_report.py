...
def _record(problem: str, n: int, tool: str, budget: str, quality: float, metric: str = "MIN_SEPARATION") -> RunRecord:
    """Build a minimal max-div budget-series record with only the fields the report reads."""
    return RunRecord(
        tool=tool,
        problem=problem,
        size=n,
...
def test_gap_table_quotes_both_series_at_both_budgets():
    """A certified cell's row names its certifiers and carries four gap cells."""
    # --- arrange ----------------------
...
def test_certification_table_states_where_each_solver_stopped():
    """Each solver's row shows its largest certified n and the size where it stopped, or that the grid ran out."""
    # --- arrange ----------------------
    exact = [
...
def test_gap_pct_is_none_without_a_value():
    """A missing budget yields no gap instead of an error."""
    # --- act / assert -----------------
